...
import { AdminCategory, useAdminCategories } from "@/lib/adminStore";

const ICON_OPTIONS = [
  "coffee","shopping-bag","heart","briefcase","tool","home",
  "truck","monitor","book-open","sun","zap","globe","music","camera","scissors",
];

export default function AdminCategoriesScreen() {
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const { data: categories, setData: setCategories } = useAdminCategories();
  ...
  const openCreate = () => {
    setEditTarget(null); setDraftName(""); setDraftIcon("briefcase"); setDraftFeatured(false);
    setShowModal(true);
  };

  const openEdit = (cat: AdminCategory) => {
    setEditTarget(cat); setDraftName(cat.name); setDraftIcon(cat.icon); setDraftFeatured(cat.featured);
    setShowModal(true);
  };

  const handleSave = () => {
    if (!draftName.trim()) { Alert.alert("Name required", "Please enter a category name."); return; }
    if (editTarget) {
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        <View style={styles.statsRow}>
          {[
            { val: categories.length, lbl: "Total",    color: colors.foreground },
            { val: activeCount,        lbl: "Active",   color: colors.accent     },
            { val: featuredCount,      lbl: "Featured", color: "#F59E0B"          },
            { val: categories.reduce((a, c) => a + c.listingCount, 0), lbl: "Listings", color: colors.primary },
          ].map(({ val, lbl, color }) => (
            <View key={lbl} style={[styles.statChip, { backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        {categories
          ...
          .map((cat) => (
            <View
              key={cat.id}
              style={[styles.catRow, { backgroundColor: colors.card, borderColor: colors.border, opacity: cat.active ? 1 : 0.6 }]}
            >
              ...
              <View style={styles.catInfo}>
                ...
                <Text style={[styles.catMeta, { color: colors.mutedForeground }]}>{cat.listingCount} listings · #{cat.sortOrder}</Text>
              </View>
              <Switch
                value={cat.active}
              ...
              />
              <TouchableOpacity onPress={() => openEdit(cat)} style={styles.iconBtn}>
