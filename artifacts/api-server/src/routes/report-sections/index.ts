...
router.get("/report-sections/csv-template/:listingId", requireAuth, async (req, res): Promise<void> => {
  ...
  try {
    ...
    // Use existing sections if present, otherwise fall back to SECTION_DEFAULTS structure
    const rows = existingSections.length > 0
      ? existingSections
      : SECTION_DEFAULTS.map((def) => ({
          id: '',
          listingId,
          ownerId: userId,
...
router.post("/report-sections/csv-import/:listingId", requireAuth, csvUpload.single("file"), async (req, res): Promise<void> => {
  ...
  try {
    ...
    for (const row of dataRows) {
      ...
      if (inclPdf !== '') {
        fields.includeInPdf = ['1', 'true', 'yes'].includes(inclPdf.toLowerCase());
        changedFields++;
      }
      if (inclHtml !== '') {
        fields.includeInHtml = ['1', 'true', 'yes'].includes(inclHtml.toLowerCase());
        changedFields++;
      }
      if (inclApp !== '') {
        fields.includeInApp = ['1', 'true', 'yes'].includes(inclApp.toLowerCase());
        changedFields++;
      }
