...
router.get("/oauth/xero/start", async (req, res) => {
  ...
  const params = new URLSearchParams({ response_type: "code", client_id: XERO_CLIENT_ID, redirect_uri: redirectUri, scope: "openid profile email offline_access accounting.reports.profitandloss.read accounting.contacts accounting.invoices accounting.banktransactions", state });
