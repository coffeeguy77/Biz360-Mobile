// chart-svg.ts — Pure TypeScript SVG string generators for all 8 IM report chart types.
// Used by:
//   • PDF generation (api-server) via svg-to-pdfkit
//   • React Native (biz360) via react-native-svg SvgXml — see biz360/components/report/chart-svg.ts
// Keep in sync with the biz360 copy when editing.
...
/** Score bars — Radar-type sections (lease_premises_summary, business_health_score) */
export function radarChartSvg(
  ...
  return svg(w, Math.max(h, rows.length * rowH), els.join(""));
}

/** Proportional distribution bars — Pie-type sections (revenue_stream_breakdown, plant_equipment_summary) */
...
/** Horizontal bar chart — H-bar sections (division_breakdown, buyer_access_confidentiality) */
export function hBarChartSvg(
  ...
  const els = rows.map((row, i) => {
    ...
    return [
      ...
      `<rect x="${labelW}" y="${y}" width="${barW}" height="9" rx="2" fill="${TRACK}"/>`,
      `<rect x="${labelW}" y="${y}" width="${filled}" height="9" rx="2" fill="${color}"/>`,
      `<text x="${labelW + barW + 4}" y="${y + 9}" font-size="9" fill="${MUTED}" font-family="Helvetica">${esc(fmtV(val))}</text>`,
    ].join("");
  });
...
/** Dispatch: produce SVG string for any of the 8 IM chart types by sectionKey */
