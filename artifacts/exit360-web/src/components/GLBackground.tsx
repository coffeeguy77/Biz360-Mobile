...
void main(){
  ...
  if(u_mode == 0){
    // ── Silky domain-warped mesh gradient ──
    vec2 p = uv * asp * 2.2;
    float t = u_time * 0.05;
    ...
    float f = fbm(p + 2.0 * r);
    float pull = 0.0;
    if(u_mouseOn > 0.5){ float d = distance(uv, m); pull = exp(-d * d * 7.0) * 0.55; r += pull * 0.35; f += pull * 0.3; }
    vec3 col = mix(u_c0, u_c1, smoothstep(0.15, 0.85, r.x));
    col = mix(col, u_c2, smoothstep(0.2, 0.85, r.y));
    col = mix(col, u_c3, smoothstep(0.25, 0.9, f));
    // Blend toward the dark base so white text over the hero stays readable,
    // brightening slightly under the cursor (pigment-pull).
    col = mix(u_bg, col, 0.5 + pull);
