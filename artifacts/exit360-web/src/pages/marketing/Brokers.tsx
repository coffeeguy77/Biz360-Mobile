...
export function Brokers() {
  const pc = usePageContent("/brokers");
  const whyI = [LayoutGrid, Smartphone, Camera, FileText, ShieldCheck, Award];
  const bigI = [Eye, FileSignature];
  const reqI = [FileText, PhoneCall, MapPin];
  const statsI = [LayoutGrid, BarChart3, Users, ShieldCheck];
  return (
    <SiteShell>
      <Seo
        ...
        jsonLd={{
          ...
          areaServed: "AU", serviceType: "Business broker software",
      ...
      <section className="theme-aurora-bg">
        <div className="relative z-10 max-w-[1440px] mx-auto px-6 pt-20 pb-24 grid lg:grid-cols-2 gap-12 items-center">
          <motion.div {...fade}>
            <span className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-full border border-primary/30 bg-primary/10 text-primary mb-5"><Briefcase size={13} /> {pc.t("hero.eyebrow")}</span>
          ...
          <motion.div {...fade} className="relative">
            ...
            <div className="relative rounded-3xl border border-border bg-card/70 backdrop-blur p-2 shadow-2xl">
              <img src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?auto=format&fit=crop&q=80&w=1000" alt="Business broker managing multiple client listings on the EXIT360 platform" className="rounded-2xl w-full h-[420px] object-cover" />
      ...
      <section className="max-w-[1440px] mx-auto px-6 py-8">
        <motion.div {...fade} className="rounded-3xl border border-border theme-aurora-bg overflow-hidden">
          <div className="relative z-10 grid lg:grid-cols-2 gap-10 p-8 sm:p-12 items-center">
            <div>
              <span className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-full border border-primary/30 bg-primary/10 text-primary mb-5"><Sparkles size={13} /> {pc.t("showcase.eyebrow")}</span>
              <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight mb-4"><RichCopy text={pc.t("showcase.heading")} /></h2>
              <p className="text-lg text-muted-foreground leading-relaxed mb-6"><RichCopy text={pc.t("showcase.body")} /></p>
              <p className="text-base text-muted-foreground leading-relaxed mb-6"><RichCopy text={pc.t("showcase.body2")} /></p>
              <Link href="/list-your-business"><Button size="lg" className="h-13 px-8 text-base theme-btn-gradient border-0">{pc.t("showcase.ctaLabel")} <ArrowRight size={18} className="ml-1" /></Button></Link>
            </div>
            <div className="rounded-2xl border border-border bg-background/70 backdrop-blur p-6 shadow-2xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div><p className="text-sm font-semibold">{pc.t("showcase.cardTitle")}</p><p className="text-xs text-muted-foreground">{pc.t("showcase.cardSub")}</p></div>
              </div>
              <div className="grid grid-cols-2 gap-4 mb-4">
                {pc.list("showcase.bigstats").map((s, i) => { const Icon = pick(bigI, i); return (
                  <div key={i} className="rounded-xl border border-border bg-card/50 p-4"><Icon className="text-primary mb-2" size={18} /><div className="text-2xl font-extrabold">{s.value}</div><div className="text-xs text-muted-foreground">{s.label}</div></div>); })}
              </div>
              <div className="space-y-3">
                {pc.list("showcase.requests").map((r, i) => { const Icon = pick(reqI, i); return (
                  <div key={i} className="flex items-center justify-between rounded-xl border border-border bg-card/40 px-4 py-3"><span className="flex items-center gap-3 text-sm"><Icon size={16} className="text-primary" /> {r.label}</span><span className="text-sm font-bold">{r.value}</span></div>); })}
              </div>
            </div>
          </div>
        </motion.div>
      ...
      <CtaBand heading={pc.t("cta.heading")} sub={pc.t("cta.sub")} primary={{ label: pc.t("cta.primary"), href: "/list-your-business" }} secondary={{ label: pc.t("cta.secondary"), href: "/selling" }} />
    </SiteShell>
  );
}
